...
main() {
    ...
    # <-- FIX 1: The package list is now a bash array.
    # This ensures each package name is passed as a separate argument to apt-get.
    local aosp_packages=(
        ...
        rsync schedtool squashfs-tools xsltproc zip zlib1g-dev gh iotop bashtop
        nethogs autossh apache2 p7zip neofetch gettext autoconf automake libtool
        make gcc autopoint patchelf python-is-python3 clang lld llvm gcc-multilib
        zsh openjdk-17-jdk openjdk-17-jre xmlstarlet micro
    )

    # <-- FIX 2: The array is expanded correctly with "${aosp_packages[@]}".
    ...
    # Manual install of ncurses5 for compatibility with older build systems
    ...
    # <-- FIX 3: Commands are now passed as separate arguments, not a single quoted string.
    ...
    # Disable AppArmor user namespace restrictions required for sandboxed builds
    run_task "Configuring AppArmor rule" sh -c "echo 'kernel.apparmor_restrict_unprivileged_userns = 0' > /etc/sysctl.d/20-apparmor-donotrestrict.conf"
